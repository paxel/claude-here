...
# into one SDK root. Platforms, build-tools and NDK are *not* baked: they are
# mounted from the host SDK (see the `android` cache), so no API level has to be
# guessed here and Gradle can install what a project asks for. Implies `jvm`.
